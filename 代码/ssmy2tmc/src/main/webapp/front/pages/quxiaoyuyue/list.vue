<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.zaotangfenlei" type="text" placeholder="澡堂分类" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                                    <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.bianhao}}</view>
                                                              <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.zaotangfenlei}}</view>
                                                                                                                                                                                                                                      <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('quxiaoyuyue','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('quxiaoyuyue','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('quxiaoyuyue','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('quxiaoyuyue','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zaotangfenlei){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`quxiaoyuyue`, params);
                } else {
                    res = await this.$api.list(`quxiaoyuyue`, params);
                }
				// 如果是第一页数据置空
			...
			async search(){
				...
				if(this.searchForm.zaotangfenlei){
					searchForm['zaotangfenlei'] = '%' + this.searchForm.zaotangfenlei + '%'
				}
				if(this.searchForm.yonghuxingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`quxiaoyuyue`, searchForm);
                } else {
                    res = await this.$api.list(`quxiaoyuyue`, searchForm);
                }
				// 如果是第一页数据置空

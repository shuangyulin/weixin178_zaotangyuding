	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zaotangfenlei`, this.ruleForm.id);
...
//跨表计算判断
				if((!this.ruleForm.zaotangfenlei)){
					this.$utils.msg(`澡堂分类不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`zaotangfenlei`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`zaotangfenlei`, this.ruleForm);
						}else{
							await this.$api.add(`zaotangfenlei`, this.ruleForm);
						}

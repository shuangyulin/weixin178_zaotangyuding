<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="澡堂分类" prop="zaotangfenlei">
          <el-select :disabled="ro.zaotangfenlei" v-model="ruleForm.zaotangfenlei" placeholder="请选择澡堂分类">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="澡堂分类" prop="zaotangfenlei">
	      <el-input v-model="ruleForm.zaotangfenlei"
                placeholder="澡堂分类" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="容纳人数" prop="rongnarenshu">
          <el-input v-model="ruleForm.rongnarenshu" 
              placeholder="容纳人数" clearable  :readonly="ro.rongnarenshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="容纳人数" prop="rongnarenshu">
              <el-input v-model="ruleForm.rongnarenshu" 
                placeholder="容纳人数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhuangtaiOptions = "已预约,可预约".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaotang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaotangCrossAddOrUpdateFlag = false;
						 this.parent.search();
